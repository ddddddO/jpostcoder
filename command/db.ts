import { Database, Statement } from "./deps.ts";
import { jpostcode } from "./jpostcode.ts";

export const insertJpostcodes = (
  db: Database,
  jpostcodes: jpostcode[],
): void => {
  const query: Statement = db.prepare(
    "INSERT INTO postcodes (code, postcode_old, postcode, ken_kana, municipalities_kana, area_kana, ken, municipalities, area, flg_a, flg_b, flg_c, flg_d, flg_e, flg_f) VALUES (:code, :postcode_old, :postcode, :ken_kana, :municipalities_kana, :area_kana, :ken, :municipalities, :area, :flg_a, :flg_b, :flg_c, :flg_d, :flg_e, :flg_f)",
  );
  db.transaction((jpostcodes: jpostcode[]) => {
    jpostcodes.map((postcode) => query.run(Object.values(postcode)));
  })(jpostcodes);
};
